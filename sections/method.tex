...
deviation, the latter floored at $\epsilon_\sigma$ (Table~\ref{tab:hparams}, App.~\ref{app:training}), and cells that are constant across
...
target frames (Table~\ref{tab:hparams}), which carry the flow interpolant and a learned marker and are the
...
$a_{\max}$ (Table~\ref{tab:hparams}), and $u,d$ are zero-initialised per-head tables. Nearer joints attend
...
kinematic. The caption is dropped with probability $p_{\mathrm{drop}}$ (Table~\ref{tab:hparams}) for
...
and $\delta$ are given in Table~\ref{tab:hparams}, and $t$ is sampled uniformly (App.~\ref{app:ablation_notes}).
...
(Table~\ref{tab:hparams}) and the others solved relative to it. A \emph{mechanism check} then takes a few
...
$\lambda_{\mathrm{vel}}$, $\lambda_{\mathrm{lock}}$ and $\lambda_{\mathrm{acc}}$ (Table~\ref{tab:hparams}), none
...
with guidance scale $s$ (Table~\ref{tab:hparams}). Twenty steps is the operating point used throughout; ten give the same retrieval at half the
...
\paragraph{Zero-training use.} An unseen rig is served by the frozen backbone as soon as its rest
...
gaits are absent from the training library.

\paragraph{Per-rig LoRA.} When roughly ten clips of the new rig exist, we attach rank-$r$
adapters~\citep{hu2022lora} with scale $\alpha$ (App.~\ref{app:lora}) to the attention projections, the MLPs
and the conditioning paths (joint-semantics projection, caption MLP, structural MLP, AdaLN), leaving the
input and output projections and the timestep MLP frozen. The adapters are folded back into the weights at
export, so the exported model is an ordinary checkpoint. The new rig is converted with the same pipeline as
the library (joint pruning to deformation bones, captions in the corpus style, an 80/20 split) and the
adapter is trained with the backbone recipe (App.~\ref{app:lora}); a single adapter shared by a group of rigs
...
tied to one skeleton. We train two towers into a shared embedding space: a frozen DistilBERT text tower
with a trainable pooling head, and a graph-aware motion tower over the \rep{} channels excluding the
contact flag, with a symmetric InfoNCE objective (App.~\ref{app:split}). The evaluator sees only real data,
never generated samples, and shares no weights with the generator.

\paragraph{Protocol.} Text$\to$motion R-precision, matching score and FID are computed in the evaluator
space and reported next to the evaluator's score on the real clips of the same pools (\S\ref{sec:exp:setup});
with shuffled captions the evaluator scores at chance (App.~\ref{app:split}). Rigs outside the library, for
which no evaluator exists, are scored geometrically (\S\ref{sec:exp:setup}).
