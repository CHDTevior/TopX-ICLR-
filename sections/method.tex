...
along time: one clean frame holding the rig's analytic rest pose, never noised, followed by the
$T{=}240$ target frames that carry the flow interpolant, a learned marker, and are the only frames
denoised and scored. Under the rest-delta parameterisation the rest frame's rotations are the identity
...
\paragraph{Three signals name the skeleton.} Nothing in the network is indexed by rig; what it knows
...
descriptions rather than bone names, because ``front left upper leg'' is the same concept on a horse
...
(rest-offset direction, bone length, tree depth, root-path length, child count, leaf flag), through a
zero-initialised MLP, added to the same tokens.

\paragraph{The graph enters as an attention bias.} Each of the 14 blocks factorises attention over the
...
$b^{(h)}_{ij}=-\min(g_{ij},8)+u^{(h)}_{a_{ij}}+d^{(h)}_{s_{ij}}$, where $g_{ij}$ is the hop distance on
the tree and $a_{ij},s_{ij}$ the hops up and down to the lowest common ancestor (clipped at 15), with
$u,d$ zero-initialised tables. Nearer joints attend more by default, and the tables let a head tell
parent from child and a joint of the same limb from one across the body (App.~\ref{app:model}). The
...
\paragraph{Text and time modulate.} $\mathbf{c}=\mathrm{MLP}(t)+\mathrm{MLP}(\mathbf{e}_c)$, with
$\mathbf{e}_c$ the pooled LLM2Vec caption embedding, drives AdaLN~\citep{peebles2023dit} shift, scale
and gate on every sub-layer and shift and scale on the output head, so text reaches the model only
through modulation and the token stream stays kinematic. The caption is dropped with probability 0.1
for classifier-free guidance; the rest-pose prompt is never dropped.

\paragraph{Scale and stability.} Width 896, 14 heads, \num{302.9}M parameters. Queries and keys are
RMS-normalised per head before the dot product~\citep{dehghani2023vit22b}: every 0.3B attempt without
it diverged, with attention logits growing from $\sim$1.4k to 23k; with it the eight probes taken
through epoch 199 of the \num{303}M run peak at \num{28.1}.
...
$\sigma_{\min}=0.2$, $\delta=10$ (below the measured 99.99th percentile of the normalised
...
$g$ in the batch and $N=\sum_g N_g$; the groups are the nine channel families of Table~\ref{tab:channels}, weighted to a target share profile~\citep{rempe2026kimodo}.
...
with the number of cells the group spans in the batch (App.~\ref{app:calib}). The shares realised during
...
a foot-lock term gated by the contact channel, and an acceleration-matching term are added, none of them
carrying the time weight $w(t)$ (App.~\ref{app:calib}).
