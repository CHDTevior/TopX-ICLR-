...
all of them (R@1 \num{0.922} against \num{0.964} for the real clips), returning rotations and a root
trajectory the rig plays directly, and a low-rank adapter on eight to twenty-six clips extends it to
rigs outside the library. The library is animal-only; retrieval comes from an evaluator we train, hence the
real-motion reference beside every number and the controls of Table~\ref{tab:controls}; a rig's statistics come
from its own clips, so a new rig needs clips or supplied ones; and the extension study is four case studies of
two to six clips.
...
hyper-parameter; Appendix~\ref{app:anytop} gives the AnyTop sampling protocol and the geometric
