...
where the prompt leaves it, the joints move \num{0.99} bone lengths per second relative to the root while it
scrambles and \num{0.20} while it lies injured.
...
profile through the five frames and the grey line is the floor beneath the root. Skeletons are the directly
predicted joint positions; the root track is shared by both decodings, so the height ranges quoted in the text
are the same read off the deployable forward-kinematic output (\num{9.21} against \num{2.08} bone lengths on
either), and only the limbs differ between the two. The quoted sentence is the conditioning prompt; the two rows of a rig differ in
