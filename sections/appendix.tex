...
\paragraph{Huber knee.} The knee $\delta=10$ lies below the measured 99.99th percentile of the normalised
targets (\num{11.45}); it acts on residuals, and at initialisation it binds on \num{0.015}\% of the supervised
cells, a figure recorded in the calibration artifact.
...
on the second temporal differences of the normalised channels (1.0), introduced against the
...
\paragraph{Hyper-parameters.} Table~\ref{tab:lora_hp} lists the per-rig settings. The Truebones arms of
Table~\ref{tab:lora} are sampled with seed 7; the zero-training arm and the adapters start from the same
backbone snapshot. The learning rate
...
act, with scale $\alpha{=}1$, on the query, key, value and output projections of both attention blocks, the
feed-forward layers and the conditioning MLPs (62 linear layers of the \num{36}M backbone, 104 of the \num{303}M
one); a
...
Table~\ref{tab:capability} lists the capabilities of the related systems as described in their papers and
released code. Two capabilities separate this task from its neighbours, and one concurrent system,
UniMate~\citep{mou2026unimate}, combines them as this paper does: text and skeleton conditioned, joint rotations
with a global root, an optional foot lock after generation, and seven held-out skeleton types scored
quantitatively. A caption is not what
...
UniMate~\citep{mou2026unimate} (concurrent) & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ (optional foot lock) & $\checkmark$ & 7 held-out skeleton types, quantitative & AnyTop with text added, AnimateAnyMesh, V2M4 \\
...
\paragraph{Retrieval on the played animation.} The evaluator of \S\ref{sec:exp:setup} scores \rep{} channels; the
...
Table~\ref{tab:hparams} gives every symbol of \S\ref{sec:method} its value. Both models train in bf16 on 8 H200
GPUs, \num{578} optimiser steps per epoch at the global batch of that table; the optimiser and schedule settings
are those of that table.

\begin{table}[h]
\centering\small\setlength{\tabcolsep}{3pt}
\caption{Symbols of \S\ref{sec:method} and their values. Two values are the \num{303}M main model / the
\num{36}M pilot; one value is shared.}\label{tab:hparams}
\begin{tabular}{@{}llr@{\hskip 10pt}llr@{}}
\toprule
\multicolumn{3}{@{}l}{Architecture, representation, conditioning} & \multicolumn{3}{l@{}}{Objective, optimisation, sampling} \\
\midrule
$D$ & width & 896 / 384 & $\sigma_{\min}$ & time-weight floor, Eq.~(\ref{eq:elem}) & 0.2 \\
$N_b$ & blocks & 14 / 8 & $\delta$ & Huber knee, Eq.~(\ref{eq:elem}) & 10 \\
$H$ & heads & 14 / 6 & $\gamma_{\mathrm{rot}}$ & rotation-family anchor & 10 \\
-- & parameters & \num{302.9}M / \num{36.3}M & $\eta$ & peak learning rate & $1.5$ / $2\times10^{-4}$ \\
$L$ & target frames & 240 & $B$ & global batch & 128 \\
$\epsilon_\sigma$ & std floor of a cell & 0.05 & -- & warm-up steps; epochs & 4k; 290 / 400 \\
$g_{\max}$ & hop-distance clip & 8 & -- & cosine floor & $10^{-2}$ over 40 epochs \\
$a_{\max}$ & LCA hop clip & 15 & -- & gradient clip; spike reject & 1.0; 200 \\
$p_{\mathrm{drop}}$ & caption dropout & 0.1 & $S$, $s$ & Euler steps, guidance & 20, 2 \\
\multicolumn{6}{@{}l@{}}{$\lambda_{\mathrm{FK}},\lambda_{\mathrm{vel}},\lambda_{\mathrm{lock}},\lambda_{\mathrm{acc}}$\quad auxiliary weights: 0.07 (ramped over the first 5k steps), 0.01, 0.01, 1.0} \\
\bottomrule
\end{tabular}
\end{table}
...
at uniform weights ($\gamma_g{=}1$ for every group, where the calibrated set anchors the rotation family at 10, so
the grouped term's weight against the auxiliary terms differs as well). It loses \num{0.036}--\num{0.043} of R@1 and its FID rises by \num{61}\% at epoch 100, and
...
\begin{table}[h]
\centering\small
\caption{Ablations on the \num{36}M pilot at matched optimiser steps (epoch 200;
...
alternative that keeps every cell's own mean and divides only by a per-rig scale (a scale-only
convention) was trained with the \num{36}M recipe unchanged and scored at matched epochs under the
...
frozen samples (the checkpoint of Table~\ref{tab:main}), decoded through the same stream and scored on
...
from it on root displacement, root path length and the two jitter rows. The
...
held-out clip of three of the four rigs: the clip of each rig whose adapted sample is nearest its held-out motion
(mean joint distance after anchoring both on the root, in mean rest bone lengths); Figure~\ref{fig:tb_main} applies
the same rule to all four rigs. Every panel overlays the two decodings of the same sample:
...
\section{Validation split and evaluator}\label{app:split}

\paragraph{Evaluator.} The two towers of \S\ref{sec:exp:setup} share a 512-d embedding space. The text tower
is a frozen DistilBERT with a trainable pooling head; the motion tower is graph-aware, with 6 graph layers and 4
temporal layers over the 16 \rep{} channels other than the contact flag; the objective is a symmetric InfoNCE
over real clips and their captions. As a control, scoring the validation clips against shuffled captions gives
R@1 \num{0.035} in pools of 32, against a chance level of \num{0.031}.

\paragraph{Split.} The library is split per rig: \num{5}\% of each rig's clips (at least one, none for a rig with a
