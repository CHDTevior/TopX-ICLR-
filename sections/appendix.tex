...
channels together and still removes the jitter excess; its gap moves by at most \num{0.13} bone lengths, and the
...
\section{Arithmetic}\label{app:controls}

Generating and scoring the epoch-100 pilot end to end with TF32 matrix multiplication
gives R@1 \num{0.8641}, against \num{0.8643} in fp32 on the same checkpoint, the same evaluation order and the
same evaluator: one clip of the \num{3{,}840} scored. R@2, R@3, the FID and the text-to-ground-truth cosine are
identical to five decimals. Table captions name the arithmetic each arm was sampled under.
...
Uncertainty for the FID is reported as the FID between two halves of the real validation set
...
everything else. It is level with the full recipe at all three epochs (\num{+0.002}, \num{+0.003},
\num{+0.002} of R@1) and its FID is \num{10}\% lower at epoch 100.
...
below the arm that keeps per-joint tokens with every skeleton pathway switched off.
...
steps, guidance 2, seed 42. The full recipe was sampled in fp32 and the other four with TF32 matrix
multiplication, which App.~\ref{app:controls} measures at \num{0.0003} of R@1. The last row is the adapted
...
validation loss, FK--pose gap, R@1 in pools of 64 and FID on the validation set). The variant without the
acceleration term optimises a different objective, so its validation loss is not listed.}\label{tab:ablation}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lcccc@{}}
\toprule
Variant & Val.\ loss$\downarrow$ & FK--pose gap, validation pass (bl)$\downarrow$ & R@1$\uparrow$ & FID$\downarrow$ \\
\midrule
...
with TF32 matrix multiplication, the \rep{} rows in fp32, which App.~\ref{app:controls} measures at
\num{0.0003} of R@1.}\label{tab:repcontent}
\begin{tabular}{lcccc}
\toprule
...
Table~\ref{tab:main} (\num{0.857} against \num{0.864}), so most of its gain over the pilot comes from
...
TF32 matrix multiplication, which App.~\ref{app:controls} measures at \num{0.0003} of R@1.}\label{tab:size}
\begin{tabular}{lcccc}
\toprule
...
the vectorised rest pose on the temporal axis as frame 0, and use the skeleton's name as the text. SAMoR
concatenates all per-joint features into a single flat per-frame vector of dimension $J_{\max}\times 6$,
zero-padded to \num{96} joints with padded positions masked in attention, and trains its baselines on the same
corpus with the same splits, loss functions and budget. OmniZoo extends MoMask and MMM with joint padding and
binary masking and appends species tags to the text prompts, on the same 95/5 split.
...
Both arms carry the same group weights, so the joint axis is the only thing that differs between them.
...
bone lengths --- equal in retrieval (R@1 \num{0.821} against \num{0.822}) and slightly better in FID
